...
# Pre-compute Mamba/Linear Hopfield Memory matrix S and norm vector z_norm
# Feature map: phi(x) = elu(x) + 1
phi_K = F.elu(prototype_tensor) + 1.0       # (10, 32)
phi_V = prototype_tensor                     # (10, 32)
S_matrix = torch.matmul(phi_K.t(), phi_V)    # (32, 32)
z_norm_vector = torch.sum(phi_K, dim=0)       # (32,)

# 4. Define Hopfield Memory-Augmented Classifier
class HopfieldClassifier(nn.Module):
    def __init__(self, encoder, memory_bank, global_mean, S_matrix, z_norm_vector, mode='linear_hopfield'):
        ...
        self.register_buffer('S_matrix', S_matrix)
        self.register_buffer('z_norm_vector', z_norm_vector)
    ...
    def forward(self, x):
        ...
        if self.mode == 'baseline':
            ...
            # Standard Softmax Hopfield retrieval: O(M) time/space
            # Cosine Centered to fix ReLU alignment
        ...
        elif self.mode == 'linear_hopfield':
            # Linear Mamba-style Hopfield retrieval: O(1) retrieval time
            phi_z = F.elu(z) + 1.0  # (batch, 32)
            
            # Retrieve from pre-computed S matrix: phi(q) * S
            numerator = torch.matmul(phi_z, self.S_matrix)  # (batch, 32)
            
            # Normalization term: phi(q) * z_norm
            denominator = torch.matmul(phi_z, self.z_norm_vector.unsqueeze(1))  # (batch, 1)
...
def train_classifier(mode, epochs=10):
    ...
    model = HopfieldClassifier(dae.encoder, prototype_tensor, global_mean_tensor, S_matrix, z_norm_vector, mode=mode).to(device)
...
modes = ['baseline', 'softmax_hopfield', 'linear_hopfield']
results = {}
...
print("\nSaved plot to hopfield_results.png")
