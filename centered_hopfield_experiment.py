...
# Pre-compute Uncentered Memory matrix and norm vector
phi_K_raw = F.elu(prototype_tensor) + 1.0
phi_V_raw = prototype_tensor
S_matrix_raw = torch.matmul(phi_K_raw.t(), phi_V_raw)
z_norm_raw = torch.sum(phi_K_raw, dim=0)

# Pre-compute Centered Memory matrix and norm vector
phi_K_centered = F.elu(prototype_tensor - global_mean_tensor) + 1.0       # (10, 32)
phi_V_centered = prototype_tensor - global_mean_tensor                     # (10, 32)
S_matrix_centered = torch.matmul(phi_K_centered.t(), phi_V_centered)       # (32, 32)
z_norm_centered = torch.sum(phi_K_centered, dim=0)                         # (32,)

# 4. Define Centered Hopfield Classifier
class CenteredHopfieldClassifier(nn.Module):
    def __init__(self, encoder, memory_bank, global_mean, 
                 S_matrix_raw, z_norm_raw, 
                 S_matrix_centered, z_norm_centered, 
                 mode='linear_hopfield_centered'):
        super(CenteredHopfieldClassifier, self).__init__()
        self.encoder = encoder
        self.mode = mode
        self.register_buffer('memory_bank', memory_bank)
        self.register_buffer('global_mean', global_mean)
        self.register_buffer('S_matrix_raw', S_matrix_raw)
        self.register_buffer('z_norm_raw', z_norm_raw)
        self.register_buffer('S_matrix_centered', S_matrix_centered)
        self.register_buffer('z_norm_centered', z_norm_centered)
    ...
    def forward(self, x):
        ...
        if self.mode == 'baseline':
            z_combined = z
        elif self.mode == 'softmax_hopfield':
            # Cosine Centered Softmax Hopfield
        ...
        elif self.mode == 'linear_hopfield_raw':
            # Uncentered Mamba-style Linear Hopfield
            phi_z = F.elu(z) + 1.0
            numerator = torch.matmul(phi_z, self.S_matrix_raw)
            denominator = torch.matmul(phi_z, self.z_norm_raw.unsqueeze(1))
            z_retrieved = numerator / (denominator + 1e-6)
            z_combined = z + z_retrieved
        elif self.mode == 'linear_hopfield_centered':
            # Centered Mamba-style Linear Hopfield
            z_centered = z - self.global_mean.unsqueeze(0)
            phi_z = F.elu(z_centered) + 1.0
            
            # Retrieve from centered S matrix
            numerator = torch.matmul(phi_z, self.S_matrix_centered)
            
            # Normalization term
            denominator = torch.matmul(phi_z, self.z_norm_centered.unsqueeze(1))
            
            z_retrieved = numerator / (denominator + 1e-6)
            
            # Add retrieved centered representation back to query
            # We reconstruct the non-centered query + retrieved offset
            z_combined = z + z_retrieved
...
def train_classifier(mode, epochs=10):
    ...
    model = CenteredHopfieldClassifier(
        dae.encoder, prototype_tensor, global_mean_tensor, 
        S_matrix_raw, z_norm_raw,
        S_matrix_centered, z_norm_centered, 
        mode=mode
    ).to(device)
...
modes = ['baseline', 'softmax_hopfield', 'linear_hopfield_raw', 'linear_hopfield_centered']
results = {}
...
plt.tight_layout()
plt.savefig('centered_hopfield_results.png')
print("\nSaved plot to centered_hopfield_results.png")

# Print Summary Table
print("\nFinal Downstream Classifier Summary (after 10 epochs):")
print("-" * 50)
print(f"{'Configuration':<25} | {'Val Loss':<10} | {'Val Acc (%)':<12}")
print("-" * 50)
for mode in modes:
    final_loss = results[mode]['val_loss'][-1]
    final_acc = results[mode]['val_acc'][-1]
    print(f"{mode:<25} | {final_loss:<10.4f} | {final_acc:<12.2f}")
print("-" * 50)
