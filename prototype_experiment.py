...
# Convert to tensor and shape (10, 32)
...
# 4. Define Memory-Augmented Classifier
class MemoryClassifier(nn.Module):
    def __init__(self, encoder, memory_bank, mode='memory'):
        ...
        if mode == 'memory':
            ...
            # Frozen random prototypes of same shape
            random_bank = torch.randn_like(memory_bank)
            self.register_buffer('memory_bank', random_bank)
    ...
    def forward(self, x):
        ...
        if self.mode in ['memory', 'random']:
            # L2 Normalize for cosine similarity calculation
            z_norm = F.normalize(z, p=2, dim=1)  # (batch, 32)
            mem_norm = F.normalize(self.memory_bank, p=2, dim=1)  # (10, 32)
            
            # Compute Cosine Similarity
            similarity = torch.matmul(z_norm, mem_norm.t())  # (batch, 10)
            
            # Softmax with temperature scale
            attention_weights = F.softmax(similarity / 0.2, dim=1)  # (batch, 10)
            
            # Retrieve from memory bank
            z_retrieved = torch.matmul(attention_weights, self.memory_bank)  # (batch, 32)
...
def train_classifier(mode, epochs=10):
    print(f"\n--- Training Downstream Classifier: {mode} ---")
    model = MemoryClassifier(dae.encoder, prototype_tensor, mode=mode).to(device)
    criterion = nn.CrossEntropyLoss()
    # Only train the classification head (fc)
...
# Run Downstream Classifier Training
modes = ['baseline', 'random', 'memory']
results = {}
...
plt.tight_layout()
plt.savefig('prototype_results.png')
print("\nSaved plot to prototype_results.png")
