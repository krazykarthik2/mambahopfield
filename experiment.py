...
# 1. Dataset Loading (MNIST - Real Dataset, using subset for speed on CPU)
...
class MiddleInputNet(nn.Module):
    def __init__(self, mode='trainable_pre'):
        ...
        if mode in ['trainable_pre', 'frozen_pre']:
            ...
            # Simple learnable bias vector instead of pre-layers
            self.bias_param = nn.Parameter(torch.zeros(128))
    ...
    def forward(self, x):
        ...
        if self.mode in ['trainable_pre', 'frozen_pre']:
            ...
            h = h_in + self.bias_param.unsqueeze(0)
        else: # baseline (no pre-layers / no additional bias)
            h = h_in
            
        return self.post_layers(h)

# 3. Training and Evaluation Helper Functions

def train_model(mode, epochs=3):
    print(f"\n--- Training Model: {mode} ---")
    model = MiddleInputNet(mode=mode).to(device)
...
for mode in modes:
    results[mode] = train_model(mode, epochs=5)
...
print("\nSaved plot to experiment_results.png")

# Print Summary Table
print("\nFinal Results Summary (after 5 epochs):")
print("-" * 50)
